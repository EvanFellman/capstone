...
argparser.add_argument('--per-question', help="How many sets per question?", type=int, default=1)
...
total_tested = 0
confidence = 0
confidence_when_right = 0
times_correct_when_confident = 0
for question_idx in tqdm.tqdm(range(min(args.num_questions, len(hotpot_df)))):
    for doc_idx in range(args.per_question):
        ...
        if output == hotpot_df['answer'].iloc[question_idx] or output[1:] == hotpot_df['answer'].iloc[question_idx]:
            points += 1
            confidence_when_right += confidence_score
        total_tested += 1
        output_data["question"].append(question)
        output_data["context"].append(text)
        output_data["quality"].append(correct_score)
        confidence += confidence_score
        print(f"Accuracy of RoBERTa given optimal documents: {points / total_tested}\t|{output}|\t|{hotpot_df['answer'].iloc[question_idx]}|\nAverage confidence of RoBERTa: {confidence / total_tested}\nAverage confidence when right: {confidence_when_right / points if points > 0 else 0}")
print(f"Accuracy of RoBERTa given optimal documents: {points / total_tested}\nAverage confidence of RoBERTa: {confidence / total_tested}\nAverage confidence when right: {confidence_when_right / points}")
